    ...
    public class EditBookedTicketHandler : IRequestHandler<EditBookedTicketCommand, EditBookedTicketResponse>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // VALIDASI: User hanya bisa edit booking milik mereka
                if (request.UserId.HasValue && bookedTicket.UserId != request.UserId)
                {
                    throw new UnauthorizedAccessException("Anda tidak memiliki akses untuk mengedit booking ini");
                }

                // Lock ticket rows untuk mencegah concurrent edit
                var ticketIds = bookedTicket.Details.Select(d => d.TicketId).ToList();
                ...
                // 2. VALIDASI untuk setiap ticket dalam request (SETELAH LOCK)
                foreach (var reqTicket in request.Tickets)
                {
                    ...
                    // VALIDASI: Quantity <= sisa quota (dengan data terkini karena sudah locked)
                    var ticket = lockedTickets.First(t => t.Id == detail.TicketId);
